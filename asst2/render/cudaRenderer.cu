...
// added on Jan/19/2019 by wenyuan
...
// Given BOX_SIZE,
// record the circles and their order in cudaDeviceCirclesInBox
// and count the number of circles in each box in cudaDeviceBoxCirclesCount,
__global__
void getCirclesInBox(int* cudaDeviceBoxCirclesCount, int* cudaDeviceCirclesInBox,
    ...
    for (int i = 0; i < circlesNum; i++){
        ...
        if (circleInBoxConservative(p.x, p.y, rad, boxL, boxR, boxT, boxB)){
            cudaDeviceCirclesInBox[boxOffset + circleCnt] = i;
            circleCnt++;
        }
    }
    // printf("circle count %d\n", circleCnt);
    cudaDeviceBoxCirclesCount[boxIndex] = circleCnt;
}

// After record the circles and their order in cudaDeviceCirclesInBox
// and count the number of circles in each box in cudaDeviceBoxCirclesCount,
// render each pixel with a thread and follow the circle orders
__global__
void kernelRenderCirclesByBox(int* cudaDeviceBoxCirclesCount, int* cudaDeviceCirclesInBox,
                              int boxNum_x, int boxNum_y){
    int imageX = blockDim.x * blockIdx.x + threadIdx.x;
    int imageY = blockDim.y * blockIdx.y + threadIdx.y;
    ...
    int boxX = imageX / BOX_SIZE;
    int boxY = imageY / BOX_SIZE;
    int boxIndex = boxY * boxNum_x + boxX;
    ...
    for (int i = 0; i < cudaDeviceBoxCirclesCount[boxIndex]; i++){
        int index = cudaDeviceCirclesInBox[boxIndex * numCircles + i];
        int index3 = 3 * index;
        float3 p = *(float3*)(&cuConstRendererParams.position[index3]);
        shadePixel(index, pixelCenterNorm, p, imgPtr);
    }
...
CudaRenderer::setup() {
    ...
    int boxNum_x = (image->width + BOX_SIZE - 1) / BOX_SIZE;
    int boxNum_y = (image->height + BOX_SIZE - 1) / BOX_SIZE;
    int numBoxs = boxNum_x * boxNum_y;
    cudaMalloc(&cudaDeviceBoxCirclesCount, sizeof(int) * numBoxs);
    cudaMalloc(&cudaDeviceCirclesInBox, sizeof(int) * numCircles * numBoxs);
...
CudaRenderer::render() {
    ...
    // for each box, check and record the circles inside it
    int boxNum_x = (image->width + BOX_SIZE - 1) / BOX_SIZE;
    int boxNum_y = (image->height + BOX_SIZE - 1) / BOX_SIZE;
    dim3 blockDim(8,8);
    ...
    getCirclesInBox<<<gridDim, blockDim>>>(cudaDeviceBoxCirclesCount, cudaDeviceCirclesInBox, boxNum_x, boxNum_y);
    cudaDeviceSynchronize();
    ...
    // printf("check circles in box DONE\n");
    printf("gridDim2: %d, %d\n", gridDim2.x, gridDim2.y);

    kernelRenderCirclesByBox<<<gridDim2, blockDim2>>>(cudaDeviceBoxCirclesCount, cudaDeviceCirclesInBox, boxNum_x, boxNum_y);
    // cudaDeviceSynchronize();
